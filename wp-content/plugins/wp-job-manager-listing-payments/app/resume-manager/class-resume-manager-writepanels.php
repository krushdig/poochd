...
/**
 * Resume Manager Writepanels
 *
 * @since 2.0.0
 */
class Resume_Manager_Writepanels {
	...
	public static function init() {
		...
		add_action( 'save_post', array( __CLASS__, 'save_meta_box_data' ), 20, 2 );
	}
	...
	public static function listing_payments_data_meta_box( $post, $box ) {
		...
		$user_packages = astoundify_wpjmlp_get_user_packages( $user_id, array( 'resume' ), true );
...
?>
	...
	<p><strong><?php esc_html_e( 'Package Product:', 'wp-job-manager-listing-payments' ) ?></strong></p>

	<p id="astoundify_wpjmlp_package_id_info">
		<?php echo wp_kses_post( $_package_id_display ); ?>
	</p>

	<?php wp_nonce_field( 'astoundify_wpjmlp_resume_save_mb_nonce', 'astoundify-wpjmlp-paid-listing-data_nonce' ); ?>
<?php
	}
	...
	public static function save_meta_box_data( $post_id, $post ) {
		...
		if ( ! isset( $request['astoundify-wpjmlp-paid-listing-data_nonce'] ) || ! wp_verify_nonce( $request['astoundify-wpjmlp-paid-listing-data_nonce'], 'astoundify_wpjmlp_resume_save_mb_nonce' ) ) {
			return $post_id;
		}
		// No Auto Save.
		...
		if ( isset( $request['_user_package_id'] ) ) {
			...
			// Get user packages.
			$user_packages = astoundify_wpjmlp_get_user_packages( $post->post_author, array( 'resume' ), true );
			...
			if ( $new_package !== $old_package ) {
				...
				if ( $new_package && array_key_exists( $new_package, $user_packages ) ) {
					...
					update_post_meta( $post_id, '_resume_duration', $user_packages[ $new_package ]->package_duration );
					...
					if ( $expire_time ) {
						update_post_meta( $post_id, '_resume_expires', $expire_time );
					}
					...
					if ( $product && 'resume_package_subscription' === $product->get_type() && 'listing' === $product->get_package_subscription_type() ) {
						update_post_meta( $post_id, '_resume_expires', '' ); // Never expire automatically.
					}
				}
				// Update meta.
				if ( $new_package ) {
