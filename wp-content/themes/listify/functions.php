...
function listify_setup() {
	...
	register_nav_menus( array(
		...
		'footer-regions'    => __( 'Footer Region Menu', 'listify' ),
	) );
...
function listify_scripts() {
	...
	wp_enqueue_script( 'listify', get_template_directory_uri() . '/js/app.min.js', $deps, 20170719, true );
...
if ( ! function_exists( 'helper_vc_fonts' ) ) {
	function helper_vc_fonts( $fonts_list ) {
	    $poppins->font_family = 'Dosis-Light';
...
function register_my_menus() {
	register_nav_menus(
		array(
		'header_navigation' => __( 'Top Navigation' )		
		)
	);
}
...
return 'hello@poochd.com';
}
function new_mail_from_name($old) {
return 'Carly';
}

/*** function for home page Listing ***/
...
echo '<section class="rating-pic"><div class="container"><div class="row">';

foreach($posts_array as $post_listing){
	
	$id = $post_listing->ID;
	
	$address = get_field('company_contact/address_details', $id);
	
	$meta = get_post_meta($id, 'rating', true);
	
	$image = wp_get_attachment_image_src(get_post_thumbnail_id($id), array('350','263'), true );
	
	$images = $image[0];
	//$image = get_the_post_thumbnail_url($id);
	$img = $images ? $images : "https://poochd.co.uk/wp-content/uploads/2018/04/thumbnail-default.jpg";

	echo '<div class="col-md-4 col-sm-4"><div class="rated-1"><a href="'.get_the_permalink($id).'"><div class="img-s"><img src="'.$img.'"></div></a>';
	echo '<a href="'.get_the_permalink($id).'"><h3>'.$post_listing->post_title.'</h3><span class="star-container">';
		...
		if($address){
			...
			echo '<p>' .get_post_meta($id, 'geolocation_formatted_address', true) . '</p>';
			}
		
		echo '<p>' .get_post_meta($id, '_phone', true) . '</p>';
		echo '</div></div>';
...
add_shortcode('home_listing', 'listing_home');
