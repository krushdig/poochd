...
<div class="job_listing-phone">
	<span itemprop="telephone"><a href="tel:<?php echo esc_attr( preg_replace( '/[^0-9,.]/', '', $telephone ) ); ?>"><?php echo esc_attr( $telephone ); ?></a></span>
</div>
...
function listify_the_listing_directions_form( $post = null ) {
	...
	$destination = $listing->get_location_formatted( $address_format );
...
<div class="listing-rating listing-rating--<?php echo esc_attr( $context ); ?>">
	...
	<?php
		...
		echo esc_html( sprintf( _n( '%d Review', '%d Reviews', $listing->get_rating_count(), 'listify' ), $listing->get_rating_count() ) );
	?>
	</span>
...
add_action( 'wp_head', 'listify_generator' );
...
function listify_get_sort_options() {
	$options = array(
		'location' => __( 'Nearest First', 'listify' ),
		'date-desc' => __( 'Newest', 'listify' ),
...
function listify_sort_listings_query( $query_args, $sort_option ) {
	...
	// elseif ( 'location' === $sort_option ) { // Random.
...
<p>
	<?php if ( get_option( 'users_can_register' ) ) : ?>
		<a href="<?php echo esc_url( wp_registration_url() ); ?>"><?php esc_html_e( 'Register', 'listify' ); ?></a>&nbsp;|&nbsp;
	<?php endif; ?>
	<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php esc_html_e( 'Lost your password?', 'listify' ); ?></a>
</p>
